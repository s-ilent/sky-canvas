...
// Function to calculate rim power
float calculateRimPower(float cloudAreaRate, float rimNarrow, float rimForce)
{
    float rimPowerR = cloudAreaRate * rimNarrow;
    rimPowerR = quadOut(saturate(rimPowerR));
    float rimPower = (1 - rimPowerR) * rimForce;
    return saturate(rimPower);
}

// Function to calculate scattering power
float calculateScatteringPower(float cloudAreaRate, float scatteringNarrow, float scatteringForce, float scatteringRange, float viewDotToLightForUv)
{
    float scatteringPower = 0;
    if (_scattering)
    {
        float scatteringPowerR = cloudAreaRate * scatteringNarrow;
        scatteringPowerR = quadOut(saturate(scatteringPowerR));
        scatteringPower = (1 - scatteringPowerR) * scatteringForce;
        float scatteringPowerRateRaw = saturate((scatteringRange - viewDotToLightForUv) / scatteringRange);
